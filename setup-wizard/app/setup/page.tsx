...
export default function SetupPage() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('pft-wizard-config')
      if (saved) setConfigState(prev => ({ ...prev, ...JSON.parse(saved) }))
      const savedStep = localStorage.getItem('pft-wizard-step')
      if (savedStep) {
  ...
  // Persist config and step to localStorage on change
  useEffect(() => {
    if (!hydrated) return
    localStorage.setItem('pft-wizard-config', JSON.stringify(config))
    localStorage.setItem('pft-wizard-step', String(currentStep))
  }, [config, currentStep, hydrated])
      ...
      <header className="border-b border-[#1e1f35] bg-[#06070d]/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              href="https://discord.com/invite/U6HjgDSmhR"
              target="_blank"
